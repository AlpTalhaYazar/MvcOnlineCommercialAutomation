...
namespace MvcOnlineCommercialAutomation.Controllers
{
    public class StatisticsController : Controller
    {
        ...
        public ActionResult Index()
        {
            ...
            ViewBag.d9 = con.Products.OrderByDescending(x => x.SalePrice).Select(y => y.ProductName).FirstOrDefault()
                .ToString(); //(from x in con.Products orderby x.SalePrice descending select x.ProductName).FirstOrDefault().ToString();
            ViewBag.d10 = con.Products.OrderBy(x => x.SalePrice).Select(y => y.ProductName).FirstOrDefault()
                .ToString(); //(from x in con.Products orderby x.SalePrice ascending select x.ProductName).FirstOrDefault().ToString();
            ...
            ViewBag.d13 = con.Products
                .Where(x => x.ProductID == con.SalesTransactions.GroupBy(y => y.ProductID)
                    .OrderByDescending(z => z.Count()).Select(a => a.Key).FirstOrDefault()).Select(b => b.ProductName)
                .FirstOrDefault();
            ...
            ViewBag.d16 = (con.SalesTransactions.Where(x => x.Date == DateTime.Today).Sum(y => (decimal?) y.Total) ?? 0)
                .ToString();
